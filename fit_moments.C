...
void fit_moments() {
  ...
  for (Int_t i=0;i<15;i++)
    hEff[i] = (TH1D*) fEff->Get(Form("hRcM%i",i));
  ...
  for (Int_t i=0;i<15;i++)
    tpwa->SetBranchAddress(Form("y%02i",i),&(vvy[i]));
  ...
  minimizer->SetFixedVariable(6 , "t6", 0.0);
  minimizer->SetFixedVariable(13, "t13", 0.0);
  minimizer->SetFixedVariable(14, "t14", 0.0);
